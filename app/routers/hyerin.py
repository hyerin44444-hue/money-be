...
router = APIRouter(prefix="/hyerin", tags=["hyerin"])

class HyerinStockCreate(BaseModel):
    name: str
    ticker: str
    quantity: float
    avg_price: float
...
class HyerinCashCreate(BaseModel):
    category: str
    amount: float
    note: Optional[str] = ""
    date: Optional[str] = None

class HyerinCash(HyerinCashCreate):
    id: str

class HyerinStock(HyerinStockCreate):
    id: str
...
def list_savings():
    rows = db.get_all_rows("hyerin_savings")
...
def add_savings(body: SavingsRecordCreate):
    ...
    db.insert_row("hyerin_savings", record.model_dump())
    return record

@router.delete("/savings/{record_id}", status_code=204)
def delete_savings(record_id: str):
    if not db.get_rows_where("hyerin_savings", id=record_id):
        raise HTTPException(status_code=404, detail="항목을 찾을 수 없습니다.")
    db.delete_row("hyerin_savings", record_id)

# ── 주식

@router.get("/stocks")
def list_stocks():
    rows = db.get_all_rows("hyerin_stocks")
    if not rows:
        return []
...
def create_stock(body: HyerinStockCreate):
    rows = db.get_all_rows("hyerin_stocks")
    ...
    stock = HyerinStock(**body.model_dump(), id=str(uuid.uuid4()))
    db.insert_row("hyerin_stocks", stock.model_dump())
    return stock

@router.put("/stocks/{stock_id}", status_code=200)
def update_stock(stock_id: str, body: HyerinStockCreate):
    if not db.get_rows_where("hyerin_stocks", id=stock_id):
        raise HTTPException(status_code=404, detail="종목을 찾을 수 없습니다.")
    db.update_row("hyerin_stocks", stock_id, body.model_dump())
    return {"ok": True}
...
def delete_stock(stock_id: str):
    if not db.get_rows_where("hyerin_stocks", id=stock_id):
        raise HTTPException(status_code=404, detail="종목을 찾을 수 없습니다.")
    db.delete_row("hyerin_stocks", stock_id)

# ── 현금

@router.get("/cash")
def list_cash():
    return db.get_all_rows("hyerin_cash")

@router.post("/cash", response_model=HyerinCash, status_code=201)
def create_cash(body: HyerinCashCreate):
    item = HyerinCash(**body.model_dump(), id=str(uuid.uuid4()))
    db.insert_row("hyerin_cash", item.model_dump())
    return item

@router.put("/cash/{item_id}", status_code=200)
def update_cash(item_id: str, body: HyerinCashCreate):
    if not db.get_rows_where("hyerin_cash", id=item_id):
        raise HTTPException(status_code=404, detail="항목을 찾을 수 없습니다.")
    db.update_row("hyerin_cash", item_id, body.model_dump())
    return {"ok": True}
...
def delete_cash(item_id: str):
    if not db.get_rows_where("hyerin_cash", id=item_id):
        raise HTTPException(status_code=404, detail="항목을 찾을 수 없습니다.")
    db.delete_row("hyerin_cash", item_id)

# ── 금

@router.get("/gold", response_model=list[GoldHolding])
def list_gold():
    return [GoldHolding(**r) for r in db.get_all_rows("hyerin_gold")]
...
def create_gold(body: GoldHoldingCreate):
    ...
    db.insert_row("hyerin_gold", item.model_dump())
    return item

@router.put("/gold/{item_id}", response_model=GoldHolding)
def update_gold(item_id: str, body: GoldHoldingCreate):
    if not db.get_rows_where("hyerin_gold", id=item_id):
        raise HTTPException(status_code=404, detail="항목을 찾을 수 없습니다.")
    db.update_row("hyerin_gold", item_id, body.model_dump())
...
def delete_gold(item_id: str):
    if not db.get_rows_where("hyerin_gold", id=item_id):
        raise HTTPException(status_code=404, detail="항목을 찾을 수 없습니다.")
    db.delete_row("hyerin_gold", item_id)
